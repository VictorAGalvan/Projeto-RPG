...
class Missao(ABC):
    ...
    def status(self, novo_estado):
        if novo_estado.__class__ not in [EstadoPendente, EstadoAndamento, EstadoConcluida, EstadoFracassada]:
            raise ExceptionGeral("Estado inválido")
        self.__status = novo_estado
    def exibir_dados(self):
        print(f"Nome missão: {self.__nome}")
        print(f"descrição: {self.__descricao}")
        print(f"Recompensa: {str(self.__recompensa)}")
        print(f"Status: {str(self.__status)}")
    def __str__(self):
        tamanho = 50
